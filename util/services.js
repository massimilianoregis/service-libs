...
var klaviyo
	var services={
		...
		get:async (url,params,jwt)=>{
			return new Promise(async (ok,ko)=>{
				...
				try{
					...
					var data = await requestify.get(url,{headers:headers,insecure: true,params:params,timeout:50000})
					ok(data.getBody())
				}
				catch(e){
					ko(e);
		...
		delete:async (url,body,jwt)=>{
			return new Promise(async (ok,ko)=>{				
				...
				try	{
					url=encodeURI(url);
					services.connections++;
					var data = await requestify.delete(url,{headers:headers,insecure: true,timeout:50000});
					ok(data.getBody())
				}
				catch(e){
					ko(e)
		...
		put:(url,body,jwt)=>{
			return new Promise(async (ok,ko)=>{
				...
				try {
					url=encodeURI(url);
					services.connections++;
					var data = await requestify.put(url,body,{headers:headers,insecure: true,timeout:50000});
					ok(data.getBody())
				}
				catch(e){
					ko();
		...
		sendNotification:(type, mail, data)=>{
			...
			console.log("klaviyo.track",type,mail,data)
		},
		externalLink:(path)=>{
